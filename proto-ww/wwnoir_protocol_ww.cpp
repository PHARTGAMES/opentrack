...
wibblewobbletrack::~wibblewobbletrack()
{
...
void wibblewobbletrack::pose(const double* headpose, const double* raw)
{
    ...
    WWHeap* const ww = pMemData;
    WWData* const data = &ww->data;
    ...
    store(data->timestamp, float(t.elapsed_seconds()));

}

QString wibblewobbletrack::game_name()
{
    QMutexLocker foo(&game_name_mutex);
    return connected_game;
}

void wibblewobbletrack::start_dummy() {
    static const QString program = OPENTRACK_BASE_PATH + OPENTRACK_LIBRARY_PATH "TrackIR.exe";
    dummyTrackIR.setProgram("\"" + program + "\"");
    dummyTrackIR.start();
}

module_status wibblewobbletrack::set_protocols()
{
...
module_status wibblewobbletrack::initialize()
{
    if (!shm.success())
        return error(tr("Can't load wibblewobble memory mapping"));
    ...
    store(pMemData->data.timestamp, 0.0f);
...
OPENTRACK_DECLARE_PROTOCOL(wibblewobbletrack, WWControls, wibblewobbleDll)
